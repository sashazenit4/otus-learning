...
BX.AbsenceCalendar.downloadReport = function(year)
{
	BX.ajax.runComponentAction('otus:vacation.schedule', 'generateReport', {
		mode: "class",
		data: {
			year: year,
		},
		...
		let a = $("<a>")
		a.attr("href", response.data.result)
		$("body").append(a)
		a.attr("download", response.data.file_name + "." + response.data.file_format)
		a[0].click()
		a.remove()
	})
}

BX.AbsenceCalendar.chooseYear = function()
{
	let contentBlock = document.createElement('form')
	contentBlock.id = 'ITEM_FORM'

	let input = document.createElement('select')
	input.classList.add('input__modal')
	input.id = 'ITEM_APPROVE_INPUT'
	...
	let nextYear = now.format('YYYY')

	for (let i = 2020; i <= nextYear; i++) {
	...
	approveButton.innerText = 'Скачать отчет'
	approveButton.id = 'ITEM_APPROVE'

	contentBlock.appendChild(input)
	buttonWrapper.appendChild(approveButton)
	contentBlock.appendChild(buttonWrapper)

	let popup = BX.PopupWindowManager.create("vacation-item-approve", null, {
	...
	approveButton = document.getElementById('ITEM_APPROVE')

	approveButton.addEventListener('click', () => {
		let year = document.getElementById('ITEM_APPROVE_INPUT')
		year = year.value
		BX.AbsenceCalendar.downloadReport(year)
		popup.close()
	})
}