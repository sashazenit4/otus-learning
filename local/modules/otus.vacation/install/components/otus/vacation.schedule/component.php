...
if ($accessManager->isUserAccountant($userId) || $USER->isAdmin()) {
	$arResult["permissions"]["CAN_DOWNLOAD_REPORT"] = true;
}
...
foreach ($arFullControlsList as $control)
{
	if (in_array($control, $arParams['FILTER_CONTROLS']))
		...
		if ($control == 'DEPARTMENT' && !$arParams['CONTROLS_DEPARTMENT_ON']) {
			unset($arResult['CONTROLS'][$control]);
		}
}
...
$absenceTypeFilter = explode(' ', Bitrix\Main\Config\Option::get('otus.vacation', 'otus_vacation_selected_types', 1));
$dbTypeRes = CIBlockPropertyEnum::GetList(array("SORT"=>"ASC", "VALUE"=>"ASC"), array('IBLOCK_ID' => $arParams['IBLOCK_ID'], 'PROPERTY_ID' => 'ABSENCE_TYPE', 'ID' => $absenceTypeFilter));
...
elseif ($arParams['AJAX_CALL'] == 'INFO')
{
	...
	if ($arParams['TYPE'] == 1 || !$calendar2)
	{
		...
		if ($arParams['TYPE'] == 1)
		{
			$arSelect = array_merge($arSelect, array('PROPERTY_USER', 'PROPERTY_FINISH_STATE', 'PROPERTY_STATE', 'PABSENCE_TYROPERTY_ABSENCE_TYPE'));
		}
		else
		{
